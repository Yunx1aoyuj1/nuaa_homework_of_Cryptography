...
public class SM2SignVerUtils {
	 ...
	 * @param privatekey 私钥
	 * @param sourceData 明文数据
	 * @param user_id 签名用户的认证
	 * @return 签名后的值
	 * @throws Exception
	 */
	public static SM2SignVO Sign2SM2(byte[] privatekey, byte[] sourceData,String user_id) throws Exception{
		SM2SignVO sm2SignVO = new SM2SignVO();
		sm2SignVO.setSm2_type("sign");
		SM2Factory factory = SM2Factory.getInstance();
		BigInteger userD = new  BigInteger(privatekey);
	 ...
	 * @return 验签的对象 包含了相关参数和验签结果
	 */
	public static SM2SignVO VerifySignSM2(byte[] publicKey, byte[] sourceData, byte[] signData,String user_id){
		try {
			byte[] formatedPubKey;
			SM2SignVO verifyVo = new SM2SignVO();
			verifyVo.setSm2_type("verify");
	...
	public static void main(String[] args) throws Exception {
		...
		SM2SignVO sign = SM2SignVerUtils.Sign2SM2(Util.hexStringToBytes(privatekey), sourceData,"ac");
		SM2SignVO verify = SM2SignVerUtils.VerifySignSM2(Util.hexStringToBytes(publicKey), sourceData, Util.hexStringToBytes(sign.getSm2_signForSoft()),"abc");
